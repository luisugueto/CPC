<?php
	...
	$articlesCategories = new Categories();
		...
		switch ($_POST['action'])
		{
			case 'submit':
				if($id != '0')
				{
					...
					echo json_encode($articlesCategories->UpdateCategory());
                    exit();
				}
				else
				{
					$articlesCategories->setName(GetSQLValueString($_POST["txtName"], "text"));
					echo json_encode($articlesCategories->CreateCategory());
                    exit();
				}
			break;
			...
			case 'form':
				...
				{
					$articlesCategories->setCategoryId($id);
					$registro = $articlesCategories->GetCategoryContent();
				}
			break;
			...
			case 'delete':
				...
				{
					$articlesCategories->setCategoryId($id);
					echo json_encode($articlesCategories->DeleteCategory());
                    exit();
				}
			break;
...
<div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button id="submitButton" class="btn btn-default waves-effect waves-light" disabled="disabled" onclick="submitModalForm('categorias_proce');"> <i class="fa fa-check m-r-5"></i> <span>ok</span> </button>
      </div>
    </div>
</div>
